...
export function listCustomers(search:string,status:string):Promise<CustomerPage>{const params=new URLSearchParams({page:'0',size:'100',sortBy:'customerName',direction:'asc'});if(search.trim())params.set('search',search.trim());if(status)params.set('status',status);return request(`/customers?${params}`);}
export const getCustomer=(id:string)=>request<Customer>(`/customers/${id}`);
export const createCustomer=(input:CustomerInput)=>request<Customer>('/customers',{method:'POST',body:JSON.stringify(input)});
export const updateCustomer=(id:string,input:CustomerInput)=>request<Customer>(`/customers/${id}`,{method:'PUT',body:JSON.stringify(input)});
export const activateCustomer=(id:string)=>request<Customer>(`/customers/${id}/activate`,{method:'POST'});
export const suspendCustomer=(id:string)=>request<Customer>(`/customers/${id}/suspend`,{method:'POST'});
export const deleteCustomer=(id:string)=>request<void>(`/customers/${id}`,{method:'DELETE'});
export const getCustomerHistory=(id:string)=>request<AuditLog[]>(`/customers/${id}/history`);
